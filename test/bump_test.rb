...
class TestBump < Test::Unit::TestCase
  ...
  def  test_help
    ...
    expectedhelp = <<-end.gsub(/^ {10}/, '')
          Usage: bump [options] [file]

          Options:
              -u, --unicode                    Emit unicode strings
              -c, --cfile FILE                 C output file name
              -m, --makefile FILE              Make output file name
              -b, --bashfile FILE              Bash output file name
              -p, --prefix PREFIX              Definition prefix
          ...
          Examples:
              MAKE EXAMPLES FROM BUMP  vvvvv
              bump -o output.cpp  foo.bin
              bump -o header.h  foo.bin
              bump -v  <foo.bin  >output.cpp

          'version' is assumed if 'file' not specified
